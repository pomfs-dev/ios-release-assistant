    ...
    <article className="launch-panel">
      <div className="launch-head">
        <div>
          ...
          <div className="launch-title">
            {text("앱 폴더를 불러오면 출시 준비 상태를 먼저 읽어드릴게요.")}
          </div>
        ...
        <span className="mini-tag">{text("첫 화면 시안")}</span>
      </div>
      ...
      <div className="launch-grid">
        <section className="setup-card emphasis">
          ...
          <div className="button-stack">
            <label className="folder-path-field">
              <span>{text("앱 폴더 또는 project.yml 경로")}</span>
              <input
                ref={folderInputRef}
                value={folderPath}
                ...
                placeholder={text("/Users/me/MyApp 또는 /Users/me/MyApp/project.yml")}
                spellCheck={false}
              />
            </label>
            <button
        ...
        <section className="setup-card">
          ...
          <h3>{text("불러온 앱에서 찾은 정보")}</h3>
          <div className={`scan-status ${scanState.status}`}>{text(scanStatusLabel(scanState))}</div>
          {scanState.status === "error" ? (
            <div className="safe-note error">{text(scanState.error)}</div>
          ) : null}
          ...
          {scanState.status === "success" ? (
            <button type="button" className="wide-button primary-soft" onClick={onContinueFromScan}>
              {pendingReviewCount > 0
                ? text(`${pendingReviewCount}개 확인 항목 보기`)
                : text("Review & Confirm으로 이동")}
            </button>
          ) : null}
        </section>
        ...
        <section className="setup-card">
          ...
          <div className="connect-box">
            <div className="connect-row">
              <span>{text("연결 상태")}</span>
              <strong>{text(appleStatusLabel)}</strong>
            </div>
            <div className="connect-row">
              <span>{text("필요한 것")}</span>
              <strong>Apple API Key</strong>
            </div>
            <div className="connect-row">
              <span>{text("비밀번호 입력")}</span>
              <strong>{text("요구하지 않음")}</strong>
            </div>
          </div>
